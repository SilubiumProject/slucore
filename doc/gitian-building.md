...
*Setup instructions for a Gitian build of Silubium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Silubium
...
- [Building Silubium Core](#building-silubium-core)
...
Clone the git repositories for silubium and Gitian.
...
```bash
sudo cp /home/debian/silubium/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the silubium repository under 'Fetch and create inputs' to install sources which require
...
To build Silubium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the silubium repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/silubium/.git/
...
For example:
```bash
URL=https://github.com/laanwj/silubium.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit silubium=${COMMIT} --url silubium=${URL} ../silubium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit silubium=${COMMIT} --url silubium=${URL} ../silubium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit silubium=${COMMIT} --url silubium=${URL} ../silubium/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-silubium:
./bin/gbuild --commit silubium=${COMMIT},cpp-eth-silubium=${ETHCOMMIT} --url silubium=${URL},cpp-eth-silubium=${ETHURL} ../silubium/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the silubium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../silubium/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/silubium
SIGPATH=/some/root/path/silubium-detached-sigs

./bin/gbuild --url silubium=${BTCPATH},signature=${SIGPATH} ../silubium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/silubium-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/silubium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/silubium-osx-build.assert
```
...
[silubiumproject/gitian.sigs](https://github.com/silubiumproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@silubium.org) and he will commit them.