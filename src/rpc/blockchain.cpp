...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // silubium
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // silubium
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // silubium
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // silubium
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination silubiumSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(silubiumSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&silubiumSenderAddress);
...
std::string fromHexAddress(std::string strHexAddress)
{
    if(strHexAddress.size()!=40)
        return "";
    else
    {
        CKeyID raw;
        raw.SetReverseHex(strHexAddress);
        CTxDestination address(raw);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of silubium addresses\n"
