...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given silubium address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The silubium address to validate\n"
            "\nResult:\n"
            "{\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"SLUPSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\"")
            + HelpExampleRpc("validateaddress", "\"SLUPSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\"")
        );
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"                       (string, required) A json array of keys which are silubium addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"                    (string) silubium address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
            ...
            + HelpExampleCli("createmultisig", "2 \"[\\\"SLUjWnDZxwLhrJDcp4Hisse8RfBo2jRDZY5Z\\\",\\\"SLU6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\"]\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("createmultisig", "2, \"[\\\"SLUjWnDZxwLhrJDcp4Hisse8RfBo2jRDZY5Z\\\",\\\"SLU6sSauSf5pF2UkUwvKGq4qjNRzBZYqgEL5\\\"]\"")
        ;
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The silubium address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"SLUD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\", \"signature\", \"my message\"")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "silubium-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nsilubium-cli has the option -getinfo to collect and format these in the old format."
    );
}
