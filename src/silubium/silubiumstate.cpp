...
SilubiumState::SilubiumState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SilubiumState::openDB(_path + "/silubiumDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SilubiumState::SilubiumState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SilubiumState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SilubiumTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSilubiumAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            silubium::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> SilubiumState::vins() const // temp
{
...
void SilubiumState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SilubiumState::vin(dev::Address const& _a) const
{
    return const_cast<SilubiumState*>(this)->vin(_a);
}

Vin* SilubiumState::vin(dev::Address const& _addr)
{
...
// void SilubiumState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SilubiumState::kill(dev::Address _addr)
{
...
void SilubiumState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address SilubiumState::createSilubiumAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void SilubiumState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SilubiumState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SilubiumState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
