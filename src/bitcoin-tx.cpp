...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " silubium-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  silubium-tx [options] <hex-tx> [commands]  Update hex-encoded silubium transaction\n" +
            "or:     silubium-tx [options] -create [commands]   Create hex-encoded silubium transaction\n" +
            "\n";
