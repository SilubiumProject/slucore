...
#include <silubium/storageresults.h>
...
using ExtractSilubiumTX = std::pair<std::vector<SilubiumTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const unsigned int DEFAULT_MIN_RELAY_TX_FEE = 10;//400000;
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 0.5*COIN;//1 * COIN;
...
#endif
static const bool DEFAULT_LOGEVENTS = true;//false;
...
static const uint64_t DEFAULT_GAS_LIMIT_OP_SEND=500000;//250000;
static const CAmount DEFAULT_GAS_PRICE=0.00000010*COIN;//0.00000040*COIN;
static const CAmount MAX_RPC_GAS_PRICE=0.00000100*COIN;

static const size_t MAX_CONTRACT_VOUTS = 1000; // silubium
...
bool CheckBlockSW(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true, bool fCheckMerkleRoot = true, bool fCheckSig=true);
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashSilubiumRoot,
    const dev::h256& oldHashStateRoot, const std::vector<SilubiumTransaction>& transactions);
...
class SilubiumTxConverter{

public:

    SilubiumTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionSilubiumTransactions(ExtractSilubiumTX& silubiumTx);
...
private:
    ...
    SilubiumTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<SilubiumTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
