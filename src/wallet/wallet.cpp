...
//                nFeeNeeded = GetMinimumFee(nBytes, coin_control, ::mempool, ::feeEstimator, &feeCalc)+nGasFee;
                ...
                if (nFeeNeeded < 10000)//::minRelayTxFee.GetFee(nBytes))最小值不可能小于0.0001
                {
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount fee_needed_with_change = GetMinimumFee(nValue,tx_size_with_change, coin_control, ::mempool, ::feeEstimator, nullptr);
...
CAmount CWallet::GetMinimumFee(CAmount total,unsigned int nTxBytes, const CCoinControl& coin_control, const CTxMemPool& pool, const CBlockPolicyEstimator& estimator, FeeCalculation *feeCalc)
{
    ...
    */
    ...
    if(total<=1000)
    {
        fee_needed =50*nTxBytes;
    }
    else if(total <=10000)
        {
        fee_needed=40*nTxBytes;
    }
    else if(total <=100000)
        {
        fee_needed=30*nTxBytes;
    }
    else if(total <=1000000)
        {
        fee_needed=20*nTxBytes;
    }
    else
        fee_needed=10*nTxBytes;

    if (feeCalc) feeCalc->reason = FeeReason::PAYTXFEE;
    fee_needed=fee_needed*total/1000;
    CAmount required_fee = 10000;//minRelayTxFee.GetFee(nTxBytes);
    if (required_fee > fee_needed) {
        fee_needed = required_fee;
        if (feeCalc) feeCalc->reason = FeeReason::REQUIRED;
    }
    // But always obey the maximum
    CAmount maximum_fee=maxTxFee;
    if (fee_needed > maximum_fee) {
        fee_needed = maximum_fee;
