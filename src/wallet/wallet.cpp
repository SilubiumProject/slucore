...
void CWallet::AvailableCoinsForStakingSW(std::vector<COutput>& vCoins,std::string address160) const
{
    vCoins.clear();
...
//        CTxDestination dest = DecodeDestination(address160);
        if (!IsValidDestination(dest)) {
            LogPrintf("Invail Silubium address:%s\n",address160);
            return;
...
int CWallet::HaveAvailableCoinsForStakingSW(std::string address160) const
{
    std::vector<COutput> vCoins;
    AvailableCoinsForStakingSW(vCoins,address160);
//    std::cout<<"Utxo Count:"<<vCoins.size()<<std::endl;
    return vCoins.size();// vCoins.size()> 0;
}
...
bool CWallet::SelectCoinsForStakingSW(CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    std::vector<COutput> vCoins;
    SilubiumDGP silubiumDGP(globalState.get(), fGettingValuesDGP);
    std::string address160=silubiumDGP.getMainNodeAddress(chainActive.Height());
    AvailableCoinsForStakingSW(vCoins,address160);
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                nFeeNeeded = GetMinimumFee(nValue, nBytes, coin_control, ::mempool, ::feeEstimator, &feeCalc)+nGasFee;
                ...
                if (nFeeNeeded < 10000)//::minRelayTxFee.GetFee(nBytes))
                {
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount fee_needed_with_change = GetMinimumFee(nValue, tx_size_with_change, coin_control, ::mempool, ::feeEstimator, nullptr)+nGasFee;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key)
{
    ...
    if(utxo_interest>0)
        txNew.vout.push_back(CTxOut(utxo_interest,txNew.vout[1].scriptPubKey));//Caluate UTXO interest
...
bool CWallet::CreateCoinStakeSW(const CKeyStore& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key)
{
    ...
    // Select coins with suitable depth
    CAmount nTargetValue = nBalance - m_reserve_balance;
    if (!SelectCoinsForStakingSW(nTargetValue, setCoins, nValueIn))
        return false;
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        ...
        // Search nSearchInterval seconds back up to nMaxStakeSearchInterval
        COutPoint prevoutStake = COutPoint(pcoin.first->GetHash(), pcoin.second);
        if (CheckKernelSW(pindexPrev, nBits, nTimeBlock, prevoutStake, *pcoinsTip, stakeCache))
        {
            // Found a kernel
            LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : kernel found\n");
            std::vector<valtype> vSolutions;
            txnouttype whichType;
            CScript scriptPubKeyOut;
            scriptPubKeyKernel = pcoin.first->tx->vout[pcoin.second].scriptPubKey;
            if (!Solver(scriptPubKeyKernel, whichType, vSolutions))
            {
                LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : failed to parse kernel\n");
                break;
            }
            LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : parsed kernel type=%d\n", whichType);
            if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
            {
                LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : no support for kernel type=%d\n", whichType);
                break;  // only support pay to public key and pay to address
            }
            if (whichType == TX_PUBKEYHASH) // pay to address type
            {
                ...
                if (!keystore.GetKey(pubKeyHash, key))
                {
                    LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : failed to get key for kernel type=%d\n", whichType);
                    break;  // unable to find corresponding public key
                }
                scriptPubKeyOut << key.GetPubKey().getvch() << OP_CHECKSIG;
            }
            if (whichType == TX_PUBKEY)
            {
                ...
                if (key.GetPubKey() != pubKey)
                {
                    LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : invalid key for kernel type=%d\n", whichType);
                    break; // keys mismatch
                }
            ...
            LogPrint(BCLog::COINSTAKE, "CreateCoinStake2 : added kernel type=%d\n", whichType);
            fKernelFound = true;
            break;
        }
    ...
    CAmount utxo_interest=CalUtxoInterest(nCredit,vwtxPrev[0]->nTimeSmart);//Caluate UTXO interest
    ...
    {
        if(!CreateMPoSOutputs(txNew, nRewardPiece, pindexPrev->nHeight, consensusParams))
            return error("CreateCoinStake2 : failed to create MPoS reward outputs");
    }
    ...
    {
        if (!SignSignature(*this, *pcoin->tx, txNew, nIn++, SIGHASH_ALL))
            return error("CreateCoinStake2 : failed to sign coinstake");
    }
...
CAmount CWallet::CalUtxoInterest(const CAmount& value,int64_t nUtxoTime)
{
    int64_t nowtime,C,day30;
    nowtime=GetTimeMicros()/1000000;
    C=2592000;//30天30*24*3600;1天24*3600;//30day
    day30=(nowtime-nUtxoTime)/C;

    if(day30<1)day30=0;
    if(day30>12)day30=12;
    return day30*value*58/10000;
}
...
void CWallet::StakeSilubiums(bool fStake, CConnman* connman)
{
    ::StakeSilubiums(fStake, this, connman, stakeThread);
}
