...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"address\"\n"
            "\nRevdumppubkeyeals the private key corresponding to 'address'.\n"
...
UniValue encryptbypubkey(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "encryptbypubkey \"pubkey\" \"information\"\n"
            "\nUsing public key to encrypt information and generate encrypted information.\n"
            "\n"
            "\nArguments:\n"
            "1. \"address\"   (string, required) The silubium address for the public key\n"
            "2. \"information\" (string,required) Information to be encrypted\n"
            "\nResult:\n"
            "\"encrypt-information\"                (string) Encryption results\n"
//            "\nExamples:\n"
//            + HelpExampleCli("dumpprivkey", "\"myaddress\"")
//            + HelpExampleCli("importprivkey", "\"mykey\"")
//            + HelpExampleRpc("dumpprivkey", "\"myaddress\"")
...
//    LOCK2(cs_main, pwallet->cs_wallet);
    ...
    std::string strAddress= request.params[0].get_str();//CBitcoinAddress(pubKey.GetID()).ToString();
    std::string strMessage = request.params[1].get_str();
    EccEncryption ec;
    return ec.Encrypt(strAddress,strMessage);


//    return HexStr(vchSig);
}

UniValue decryptbyprivkey(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "decryptbyprivkey \"address\" \"encrypt-information\"\n"
            "\nDecrypt the information with the private key.\n"
            "\n"
            "\nArguments:\n"
            "1. \"address\"   (string, required) The silubium address for the public key\n"
            "2. \"encrypt-information\" (string,required) Encrypt information.\n"
            "\nResult:\n"
            "\"information\"                (string) Decipher information.\n"
//            "\nExamples:\n"
//            + HelpExampleCli("dumpprivkey", "\"myaddress\"")
//            + HelpExampleCli("importprivkey", "\"mykey\"")
//            + HelpExampleRpc("dumpprivkey", "\"myaddress\"")
    ...
    std::string strMessage = request.params[1].get_str();
    JSONRPCRequest req;
    req.params.push_back(strAddress);
//    req.strMethod = "dumpprivkey";
    std::string strprivkey=dumpprivkey(req).get_str();


    EccEncryption ec;
    return ec.Decrypt(strprivkey,strMessage);
