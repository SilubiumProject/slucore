...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The silubium address to be associated with an account.\n"
            "2. \"account\"         (string, required) The account to assign the address to.\n"
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"SLUD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"SLUD1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\", \"tabby\"")
        );
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start silubiumd with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A silubium address associated with the keys.\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3)) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending silubiums\n"
            "\nArguments:\n"
...
UniValue walletlock(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() != 0)) {
        throw std::runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"SLUM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 1.0") +
...
{ //  category              name                        actor (function)           argNames
    ...
    { "util",               "dumppubkey",               &dumppubkey,               {"address"} },
};
