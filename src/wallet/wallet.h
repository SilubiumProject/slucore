...
//! -mintxfee default
static const CAmount DEFAULT_TRANSACTION_MINFEE = 10000;//400000;
...
static const bool DEFAULT_USE_CHANGE_ADDRESS = false;//true;
...
private:
    ...
    void StakeSilubiums(bool fStake, CConnman* connman);
...
public:
    ...
    bool SelectCoinsForStakingSW(CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const;
	
    /**
     * populate vCoins with vector of available COutputs.
     */
    void AvailableCoinsForStaking(std::vector<COutput>& vCoins) const;
    void AvailableCoinsForStakingSW(std::vector<COutput>& vCoins,std::string address160) const;
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlySafe=true, const CCoinControl *coinControl = nullptr, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY, const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t nMaximumCount = 0, const int nMinDepth = 0, const int nMaxDepth = 9999999) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool HaveAvailableCoinsForStaking() const;
    int HaveAvailableCoinsForStakingSW(std::string address160) const;
    ...
    bool CreateCoinStakeSW(const CKeyStore &keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key);
    CAmount CalUtxoInterest(const CAmount& value,int64_t nUtxoTime);//Caluate UTXO interest
    ...
    /* Start staking silubiums */
    void StartStake(CConnman* connman) { StakeSilubiums(true, connman); }

    /* Stop staking silubiums */
    void StopStake() { StakeSilubiums(false, 0); }
