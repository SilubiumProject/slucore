...
inline void initState(){
    ...
    const std::string dirSilubium = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<SilubiumState>(new SilubiumState(dev::u256(0), SilubiumState::openDB(dirSilubium, hashDB, dev::WithExisting::Trust), dirSilubium + "/silubiumDB", dev::eth::BaseState::Empty));
...
inline dev::Address createSilubiumAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline SilubiumTransaction createSilubiumTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    SilubiumTransaction txEth;
    if(recipient == dev::Address()){
        txEth = SilubiumTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = SilubiumTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<SilubiumTransaction> txs){
