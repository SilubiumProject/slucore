...
#include <silubiumtests/test_utils.h>
...
BOOST_AUTO_TEST_CASE(bytecodeexec_txs_empty){
    initState();
    std::vector<SilubiumTransaction> txs;
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    SilubiumTransaction txEth = createSilubiumTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txs(1, txEth);
    auto result = executeBC(txs);
    
    std::vector<dev::Address> addrs = {createSilubiumAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasBase){
    initState();
    SilubiumTransaction txEth = createSilubiumTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    SilubiumTransaction txEth = createSilubiumTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasBase_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        SilubiumTransaction txEth = createSilubiumTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createSilubiumAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        SilubiumTransaction txEth = createSilubiumTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createSilubiumAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        SilubiumTransaction txEth = createSilubiumTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createSilubiumAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    SilubiumTransaction txEthCreate = createSilubiumTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    std::vector<dev::Address> addrs = {createSilubiumAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    SilubiumTransaction txEthCall = createSilubiumTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<SilubiumTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasBase_return_value){
    ...
    std::vector<SilubiumTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createSilubiumAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    SilubiumTransaction txEthCall = createSilubiumTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<SilubiumTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    SilubiumTransaction txEthCreate = createSilubiumTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createSilubiumAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    SilubiumTransaction txEthCall = createSilubiumTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<SilubiumTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    executeBC(txs);
    std::vector<SilubiumTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        SilubiumTransaction txEthCall = createSilubiumTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        SilubiumTransaction txEth = createSilubiumTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createSilubiumAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        SilubiumTransaction txEthCall = createSilubiumTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<SilubiumTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        SilubiumTransaction txEthCreate = createSilubiumTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        SilubiumTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createSilubiumTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createSilubiumAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createSilubiumAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        SilubiumTransaction txEthCall = createSilubiumTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    SilubiumTransaction txEthCreate = createSilubiumTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<SilubiumTransaction> txs(1, txEthCreate);
    executeBC(txs);
    std::vector<SilubiumTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createSilubiumAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        SilubiumTransaction txEthCall = createSilubiumTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
