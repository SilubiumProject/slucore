...
void checkResult(bool isCreation, std::vector<SilubiumTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    SilubiumTxConverter converter(transaction, NULL);
    ExtractSilubiumTX silubiumTx;
    BOOST_CHECK(converter.extractionSilubiumTransactions(silubiumTx));
    std::vector<SilubiumTransaction> result = silubiumTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(silubiumtxconverter_tests, TestingSetup)
