...
bool ToHash160(const std::string& strSilubiumAddress, std::string& strHash160)
{
    CTxDestination silubiumAddress = DecodeDestination(strSilubiumAddress);
    if(!IsValidDestination(silubiumAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&silubiumAddress);
    if(keyid){
...
bool ToSilubiumAddress(const std::string& strHash160, std::string& strSilubiumAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination silubiumAddress = keyid;
    if(IsValidDestination(silubiumAddress)){
        strSilubiumAddress = EncodeDestination(silubiumAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToSilubiumAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToSilubiumAddress(tokenEvent.receiver, tokenEvent.receiver);
