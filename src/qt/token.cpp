...
{
    CBitcoinAddress silubiumAddress(strSilubiumAddress);
    if(silubiumAddress.IsValid()){
        CKeyID keyid;
        silubiumAddress.GetKeyID(keyid);
...
bool ToSilubiumAddress(const std::string& strHash160, std::string& strSilubiumAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress silubiumAddress;
    silubiumAddress.Set(keyid);
    if(silubiumAddress.IsValid()){
        strSilubiumAddress = silubiumAddress.ToString();
        return true;
    }
    return false;
}
