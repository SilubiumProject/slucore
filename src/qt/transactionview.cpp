    ...
    transactionView(0), abandonAction(0), bumpFeeAction(0), columnResizingFixer(0)
{
    ...
    typeWidget->addItem(tr("Interest"), TransactionFilterProxy::TYPE(TransactionRecord::Interest));
    typeWidget->addItem(tr("GasChange"), TransactionFilterProxy::TYPE(TransactionRecord::GasChange));
    ...
    if(_model)
    {
        ...
        if (_model->getOptionsModel())
        {
            ...
            for (int i = 0; i < listUrls.size(); ++i)
            {
                ...
                if (!host.isEmpty())
                {
                    QAction *thirdPartyTxUrlAction = new QAction(tr("BlockChain Explorer"),this);//new QAction(host, this); // use host as menu item label
                    if (i == 0)
                        contextMenu->addSeparator();
