    ...
    if (nNet > 0 || wtx.is_coinbase || wtx.is_coinstake)
    {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if(mine)
            {
                ...
                {
                    if(wtx.tx->vout.size()>7)//TODO:chainparams.GetConsensus().nMPoSRewardRecipients+2)
                    {
                        CTxDestination addressBit;
                        txnouttype txType=TX_NONSTANDARD;
                        for(int j=(wtx.tx->vout.size()-1);j>6;j--)//TODO:chainparams.GetConsensus().nMPoSRewardRecipients+1
                        {
                            if(ExtractDestination(wtx.tx->vout[j].scriptPubKey, addressBit, &txType))
                            {
                                isminetype ismine = wtx.txout_is_mine[j];
                                if(ismine)
                                {
                                    if(txType == TX_PUBKEY)
                                    {
                    ...
                    else if(wtx.tx->vout.size()==4)
                    {
                        sub.type= TransactionRecord::GasChange;
                        sub.gaschange+=wtx.tx->vout[3].nValue;
                    }
                    else
                        sub.type = TransactionRecord::Generated;
                }
    ...
    else
    {
        ...
        if (fAllFromMe && fAllToMe)
        {
            ...
            parts.append(TransactionRecord(hash, nTime, TransactionRecord::SendToSelf, "",
                            -(nDebit - nChange), nCredit - nChange,0,0));
        ...
        else
        {
            ...
            //
            parts.append(TransactionRecord(hash, nTime, TransactionRecord::Other, "", nNet, 0,0,0));
