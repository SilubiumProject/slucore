...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Silubium address (e.g. %1)").arg("SLUUjkxdPXkpp3hNLoP5ZxKty9b46QUsXslu"));
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("silubium"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("silubium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "slu:"); // SILUBIUM: change string length to 7 to correctly parse silubium://
    }
...
{
    QString ret = QString("slu:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Silubium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Silubium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Silubium (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "silubium.desktop";
    return GetAutostartDir() / strprintf("silubium-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Silubium\n";
        else
            optionFile << strprintf("Name=Silubium (%s)\n", chain);
