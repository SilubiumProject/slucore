...
QVariant TransactionTableModel::txAddressDecoration(const TransactionRecord *wtx) const
{
    ...
    case TransactionRecord::GasChange:
        return platformStyle->TableColorIcon(":/icons/gaschange",PlatformStyle::Inout);
    case TransactionRecord::Interest:
        return platformStyle->TableColorIcon(":/icons/interest", PlatformStyle::Input);
    case TransactionRecord::RecvWithAddress:
    case TransactionRecord::RecvFromOther:
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, BitcoinUnits::SeparatorStyle separators) const
{
    ...
    switch (wtx->type) {
    case TransactionRecord::Interest:
        str = BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->interest, false, separators);
        if(showUnconfirmed)
        {
            if(!wtx->status.countsForBalance)
            {
        ...
        break;
    case TransactionRecord::GasChange:
        str = BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->gaschange, false, separators);
        if(showUnconfirmed)
        {
            if(!wtx->status.countsForBalance)
            {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case AmountRole:
        ...
        {
        case TransactionRecord::Interest:
            return qint64(rec->interest);
            break;
        case TransactionRecord::GasChange:
            return qint64(rec->gaschange);
            break;
        default:
            return qint64(rec->credit + rec->debit);
            break;
