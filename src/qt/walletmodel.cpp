...
bool WalletModel::isUnspentAddress(const std::string &silubiumAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    for (const COutput& out : vecOutputs)
    {
        ...
        if(fValidAddress && CBitcoinAddress(address).ToString() == silubiumAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
