...
bool BCLog::Logger::OpenDebugLog()
{
    ...
    assert(m_fileoutVM == nullptr); // silubium
    assert(!m_file_path.empty());
    assert(!m_file_pathVM.empty()); // silubium
...
std::string BCLog::Logger::LogTimestampStr(const std::string &str)
{
    ...
    if (m_started_new_line) {
        ...
        strStamped = FormatISO8601DateTime(nTimeMicros/1000000+28800);//日志时间调为东8区
