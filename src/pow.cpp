...
unsigned int GetNextWorkRequiredSW(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params, bool fProofOfStake)
{
    ...
    return pindexPrevPrev->nBits;
//    return nTargetLimit;
//    return CalculateNextWorkRequiredSW(pindexPrev, pindexPrevPrev->GetBlockTime(), params, !fProofOfStake);
}
...
unsigned int CalculateNextWorkRequiredSW(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params, bool fProofOfStake)
{
    if(fProofOfStake){
        if (params.fPoSNoRetargeting)
            return pindexLast->nBits;
