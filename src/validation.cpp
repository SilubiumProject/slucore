                                     ...
                                     bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = silubiumDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
        ...
        if (fLimitFree && nModifiedFees < 10000){//::minRelayTxFee.GetFee(nSize)) {
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ByteCodeExec exec(block, std::vector<SilubiumTransaction>(1, callTransaction), blockGasLimit);
                         ...
                         CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertSilubiumTX.first, blockGasLimit);
    ...
    //    if(pindex->nHeight > 10000 && !Params().GetConsensus().fPoSNoRetargeting) {
    //        //sanity check in case an exploit happens that allows new coins to be minted
    //        //Silubium para
    //        //if(pindex->nMoneySupply > (uint64_t)(100000000 + ((pindex->nHeight - 5000) * 4)) * COIN){//qtum
    //        if(pindex->nMoneySupply > (uint64_t)(100000000 + ((pindex->nHeight - 10000) * 1)) * COIN){
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
              ...
              DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()+28800),
...
bool LoadGenesisBlock(const CChainParams& chainparams)
{
    ...
    try {
        ...
        //               hashGenesisBlock=UintToArith256(block.GetHash());
