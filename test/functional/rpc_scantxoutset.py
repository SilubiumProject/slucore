...
from test_framework.silubiumconfig import COINBASE_MATURITY
from test_framework.silubium import convert_btc_address_to_silubium
...
class ScantxoutsetTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mkHV1C6JLheLoUSSZYk7x3FH5tnx9bu7yc"), 0.008) # (m/0'/0'/0')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mipUSRmJAj2KrjSvsPQtnP8ynUon7FhpCR"), 0.016) # (m/0'/0'/1')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("n37dAGe6Mq1HGM9t4b6rFEEsDGq7Fcgfqg"), 0.032) # (m/0'/0'/1500')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mqS9Rpg8nNLAzxFExsgFLCnzHBsoQ3PRM6"), 0.064) # (m/0'/0'/0)
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mnTg5gVWr3rbhHaKjJv7EEEc76ZqHgSj4S"), 0.128) # (m/0'/0'/1)
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mketCd6B9U9Uee1iCsppDJJBHfvi6U6ukC"), 0.256) # (m/0'/0'/1500)
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mj8zFzrbBcdaWXowCQ1oPZ4qioBVzLzAp7"), 0.512) # (m/1/1/0')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mfnKpKQEftniaoE1iXuMMePQU3PUpcNisA"), 1.024) # (m/1/1/1')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mou6cB1kaP1nNJM1sryW6YRwnd4shTbXYQ"), 2.048) # (m/1/1/1500')
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mtfUoUax9L4tzXARpw1oTGxWyoogp52KhJ"), 4.096) # (m/1/1/0)
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mxp7w7j8S1Aq6L8StS2PqVvtt4HGxXEvdy"), 8.192) # (m/1/1/1)
        self.nodes[0].sendtoaddress(convert_btc_address_to_silubium("mpQ8rokAhp1TAtJQR6F6TaUmjAWkAWYYBq"), 16.384) # (m/1/1/1500)
